...
@Slf4j
data class CommonResp<T>(val timestamp: Date, val status: Int, val message: String, val path: String?, val data: T) {
    ...
    companion object {
        ...
        fun <T> of(status: HttpStatus, data: T, message: String?): CommonResp<T> {
            ...
            return CommonResp(Date(), status.value(), message ?: DEFAULT_MESSAGE, path, data)
        }
    }
}
